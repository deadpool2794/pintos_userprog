...
static void push_arguments_onto_stack (esp_t *esp, char *args_str, char *save_ptr);
/* Deny write to self file */
static bool deny_write_to_process_file (struct thread *cur, const char *name);
/* Recover the deny for writing to self */
static void recover_write_to_process_file (struct thread *cur);
...
process_execute (const char *file_name)
{
  ...
  /* split with " " */
  char *save_ptr;
  exact_file_name = strtok_r (exact_file_name, " ", &save_ptr);

  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create (exact_file_name, PRTY_DEFAULT, start_process, fn_copy);
  /* Prevent memory leak */
  if (tid == TID_ERROR)
    palloc_free_page (fn_copy);
  ...
  if (!success)
    {
      ...
      cur->process->status = 3;
      sem_up (&cur->process->load_sem);
      thread_exit ();
    }
  else
    {
      push_arguments_onto_stack ((esp_t *)&if_.esp, token, save_ptr);

      bool deny_success = deny_write_to_process_file (cur, token);
      palloc_free_page (file_name);
      if (deny_success)
        {
...
process_exit (void)
{
  ...
  if (cur->process->status != 3)
    cur->process->status = 1;
  /* Recover write */
  recover_write_to_process_file (cur);
...
/* Parse arguments for the stack */
static void
push_arguments_onto_stack (esp_t *esp, char *args_str, char *save_ptr)
{
...
/* Deny write to self file */
static bool
deny_write_to_process_file (struct thread *cur, const char *name)
{
  /* Open self file */
...
/* Recover the deny for writing to self */
static void
recover_write_to_process_file (struct thread *cur)
{
